// Avi Karaoke Studio — v1.144 fast resilient loader
(async()=>{
  const started=performance.now();
  window.__hksLoaderVersion=144;
  ...
  if(ver)ver.textContent='Web v1.144 — טוען…';

  const warmUrls=patches.slice(6).map(n=>`./patch-v${n}.js?v=144`);
  let nextWarm=0;
  ...
  async function loadPatch(n){
    ...
    for(let attempt=1;attempt<=3;attempt++){
      try{
        ...
        await import(`./patch-v${n}.js?v=144${suffix}`);
        return true;
      }catch(e){
        lastErr=e;
        console.warn(`[v144 loader] patch ${n} attempt ${attempt} failed`,e);
        await sleep(150*attempt);
      }
    }
    window.__hksLoaderFailedPatches.push(n);
    console.error(`[v144 loader] patch ${n} failed after retries`,lastErr);
    return false;
  }

  for(let i=0;i<patches.length;i++){
    await loadPatch(patches[i]);
    if(ver && (i%12===0 || i===patches.length-1)) ver.textContent=`Web v1.144 — ${Math.round((i+1)/patches.length*100)}%`;
  }

  window.__hksStartupMs144=Math.round(performance.now()-started);
  const failed=window.__hksLoaderFailedPatches;
  if(failed.length){
    if(ver)ver.textContent=`Web v1.144 (${failed.length} קבצים חסרים)`;
    try{setStatus(`עדכון v1.144 נטען חלקית: ${failed.join(', ')}`)}catch(_){}
  }else{
    if(ver)ver.textContent='Web v1.144';
    try{setStatus(`v1.144 מוכן — עלה ב-${(window.__hksStartupMs144/1000).toFixed(1)} שניות.`)}catch(_){}
  }
})();