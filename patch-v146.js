...
(function () {
  ...
  // Never expose Safari's white backing surface during repaint.
  try {
    ...
    if (!$('#hksIOSPaint146')) {
      ...
      st.id = 'hksIOSPaint146';
      st.textContent = 'html,body{background:#050d16!important;min-height:100%;}body{-webkit-text-size-adjust:100%;}';
  ...
  if (renderBtn && !mode) {
    ...
    box.id = 'hksIPhoneRenderBox146';
    box.style.cssText = 'margin:10px 0;padding:11px;border:1px solid #35506a;border-radius:12px;background:#0b1622;direction:rtl';
    box.innerHTML = '<div style="font-weight:900;color:#f2cf79;margin-bottom:7px">מצב רינדור באייפון</div>' +
      '<select id="hksIPhoneRenderMode146" style="width:100%;min-height:48px;border-radius:10px;background:#17131d;color:#fff;border:2px solid #3b3144;padding:8px;font-weight:900">' +
      '<option value="safe720" selected>מומלץ — 720p HD יציב ומהיר</option><option value="1080">1080p Master — כבד / ניסיוני באייפון</option></select>' +
      '<div id="hksIPhoneRenderHint146" style="font-size:12px;color:#b8c7d6;margin-top:7px;line-height:1.45">720p/30fps • MP4 עד 6 Mbps + AAC 320 kbps • WMV 4 Mbps + PCM לא דחוס 48kHz/16-bit/Stereo.</div>';
    renderBtn.parentElement?.insertBefore(box, renderBtn);
    mode = $('#hksIPhoneRenderMode146');
  }
  ...
  exportPreset = function () {
    ...
    if (isSafe()) {
      ...
      p.width = Math.min(1920, Number(p.width) || 1920);
      p.height = 1080;
      p.fps = Math.min(30, Number(p.fps) || 30);
      if (p.videoK === '20M') p.videoK = '12M';
    }
  ...
  window.wmvExportPreset = function () {
    ...
    if (isSafe()) {
      p.width = 1280; p.height = 720; p.fps = 30; p.videoK = '4M';
    } else {
      p.width = Math.min(1920, Number(p.width) || 1920);
      p.height = 1080;
      p.fps = Math.min(30, Number(p.fps) || 30);
  ...
  function rememberLabel(opt, key) {
    if (opt && !opt.dataset[key]) opt.dataset[key] = opt.textContent || '';
  }
  function restoreLabels(sel, key) {
    if (!sel) return;
    [...sel.options].forEach(o => {
      if (o.dataset[key] && o.textContent !== o.dataset[key]) o.textContent = o.dataset[key];
    });
  }
  function setSelectedLabel(sel, label, key) {
    if (!sel) return;
  ...
  function estimate146() {
    ...
    if (!(d > 0)) return;
    const mv = isSafe() ? 6 : 12;
    const wv = isSafe() ? 4 : 12;
  ...
  function syncVisibleSettings() {
    const mp4v = $('#mp4Video') || $('#videoQuality');
    const wmvv = $('#wmvVideo') || mp4v;
    const mp4a = $('#mp4Audio') || $('#audioQuality');
    const wmva = $('#wmvAudio') || mp4a;
    [mp4v, wmvv].forEach(s => restoreLabels(s, 'hksOrig146v'));
    [mp4a, wmva].forEach(s => restoreLabels(s, 'hksOrig146a'));
    if (isSafe()) {
    ...
    const h = $('#hksIPhoneRenderHint146');
    if (h) h.textContent = isSafe() ? '720p/30fps • MP4 עד 6 Mbps + AAC 320 kbps • WMV 4 Mbps + PCM לא דחוס 48kHz/16-bit/Stereo.' : '1080p/30fps ניסיוני • MP4 AAC 320 kbps • WMV PCM לא דחוס.';
    estimate146();
  }
  ...
  document.addEventListener('change', e => {
    if (['mp4Video', 'wmvVideo', 'mp4Audio', 'wmvAudio'].includes(e.target?.id)) setTimeout(estimate146, 0);
  }, true);

  // Export filename spaces.
  function bindFilename() {
    const input = $('#exportFileName');
    if (!input || input.dataset.hksSpace146) return;
    input.dataset.hksSpace146 = '1';
    const insertSpace = () => {
      const s = Number.isFinite(input.selectionStart) ? input.selectionStart : input.value.length;
      const e = Number.isFinite(input.selectionEnd) ? input.selectionEnd : s;
      if (typeof input.setRangeText === 'function') input.setRangeText(' ', s, e, 'end');
  ...
  function setArg(a, key, val) {
    const i = a.indexOf(key);
    if (i >= 0 && i + 1 < a.length) a[i + 1] = String(val);
    else a.splice(Math.max(0, a.length - 1), 0, key, String(val));
  }
  function removeArg(a, key, withValue = true) {
    let i;
    while ((i = a.indexOf(key)) >= 0) a.splice(i, withValue ? 2 : 1);
  }
  function sanitizeCommand(args) {
    const a = [...args];
    const out = String(a[a.length - 1] || '').toLowerCase();
    if (out.endsWith('.mp4')) {
      ...
      if (isSafe()) setArg(a, '-b:v', '4M');
      const w = a.indexOf('wmv2');
      if (w >= 0 && !a.includes('-threads')) a.splice(w + 1, 0, '-threads', '1');
    }
    return a;
  }
  ...
  async function newEngine() {
    ...
    const mods = await timeout(Promise.all([
      import('./vendor/ffmpeg/ffmpeg/ios.js?v=146'),
      import('./vendor/ffmpeg/util/index.js?v=146')
    ]), 30000, 'לא הצלחתי לטעון את קבצי FFmpeg');
    const { FFmpeg } = mods[0];
    const { fetchFile } = mods[1];
    const f = new FFmpeg();
    const errors = [];
    let lastPaint = 0, lastProgress = 0;

    f.on('log', ({ message }) => {
      const m = String(message || '').trim();
      if (/error|failed|invalid|memory|abort|cannot|unable|allocation|killed|fatal/i.test(m)) {
        errors.push(m); if (errors.length > 8) errors.shift(); window.__hksLastFFmpeg146 = m;
      }
    });
    f.on('progress', ({ progress }) => {
      const now = Date.now();
      const v = Math.max(0, Math.min(1, Number(progress) || 0));
      if (now - lastPaint < 350 && v < .995 && v >= lastProgress) return;
      lastPaint = now; lastProgress = v;
      try {
        if (renderStage === 'mp4') setExportState('שלב 2/4 — מרנדר MP4…', 18 + v * 50);
        else if (renderStage === 'wmv') setExportState('שלב 3/4 — יוצר WMV…', 80 + v * 16);
      } catch (_) {}
    });
    ...
    const rawRead = f.readFile.bind(f);

    f.exec = async (args, limit = -1) => {
      const a = sanitizeCommand(args);
      const out = String(a[a.length - 1] || '').toLowerCase();
      ...
      if (rc !== 0) {
        const detail = errors.slice(-3).join(' | ') || window.__hksLastFFmpeg146 || ('FFmpeg code ' + rc);
  ...
  loadFFmpeg = async function () {
    ...
    window.__hksFFmpegLoading146 = (async () => {
      let last = null;
      for (let i = 1; i <= 2; i++) {
        try {
          return await newEngine();
        } catch (e) {
          last = e;
          try { ffmpegInstance?.terminate?.(); } catch (_) {}
          ffmpegInstance = null;
          if (i < 2) {
            try { setExportState('הפתיחה הראשונה נכשלה — מנסה שוב…', 2); } catch (_) {}
            await sleep(700);
          }
        }
      }
    ...
    try { return await window.__hksFFmpegLoading146; }
    finally { window.__hksFFmpegLoading146 = null; }
  };

  // Smart update button, no permanent observers and no self-update loop.
  async function latestVersion() {
    try {
      const r = await fetch('./version.json?t=' + Date.now(), { cache: 'no-store' });
      if (!r.ok) return null;
      const j = await r.json(), v = Number(j?.version || 0);
      return Number.isFinite(v) && v > 0 ? v : null;
    } catch (_) { return null; }
  }
  async function doUpdate(btn) {
    if (window.__hksUpdating146) return;
    window.__hksUpdating146 = true;
    ...
    try {
      ...
      if (latest <= 146) { try { setStatus('אתה כבר בגרסה האחרונה — v1.146.'); } catch (_) {} return; }
      ...
      window.__hksUpdating146 = false;
      if (btn) { btn.disabled = false; btn.textContent = old; }
    }
  }
  function bindUpdate() {
    const btn = $('#hksRefresh105') || [...document.querySelectorAll('button')].find(b => /רענן\s*עדכון|עדכון\s*רענן|מעדכן\s*לגרסה|בודק\s*עדכון/.test(String(b.textContent || '')));
    if (!btn || btn.dataset.hksUpdate146) return;
    btn.dataset.hksUpdate146 = '1';
    btn.addEventListener('click', e => { e.preventDefault(); e.stopImmediatePropagation(); doUpdate(btn); }, true);
  }
  ...
  try {
    ...
    if (u.searchParams.has('hksUpdate') || u.searchParams.has('_')) {
      u.searchParams.delete('hksUpdate'); u.searchParams.delete('_'); history.replaceState(null, '', u.pathname + u.search + u.hash);
    }
  } catch (_) {}
