...
(function(){
  ...
  if(!document.getElementById('hksPreviewParityStyle152')){
    ...
    st.textContent=`
      ...
      #preview:fullscreen,#preview:-webkit-full-screen{
        ...
        margin:auto!important;border-radius:0!important;background:#000!important;
      }
    `;
  ...
  function metric(el){
    ...
    return {fontPx:fs,fontH:fs/pr.height,lineRatio:lineRatio(cs,fs),left:(r.left-pr.left)/pr.width,right:(pr.right-r.right)/pr.width,top:(r.top-pr.top)/pr.height,centerX:((r.left+r.width/2)-pr.left)/pr.width,centerY:((r.top+r.height/2)-pr.top)/pr.height,width:r.width/pr.width,textShadow:cs.textShadow||'none'};
  }
  function capture(force=false){
    ...
    last={width:pr.width,height:pr.height,lyrics:metric(lyrics),brandL:metric(brandL),brandR:metric(brandR),title:metric(titleText),capturedAt:Date.now()};
    window.__hksPreviewMetrics152=last;return last;
  }
  ...
  function scaleShadow(shadow,f){if(!shadow||shadow==='none'||!Number.isFinite(f)||f<=0)return shadow||'none';return String(shadow).replace(/(-?\d*\.?\d+)px/g,(m,n)=>(Number(n)*f).toFixed(2)+'px')}
  function applyMetric(el,m,pr,kind){
    if(!el||!m)return;
    const factor=pr.height/Math.max(1,last?.height||pr.height);
    el.style.fontSize=(m.fontH*pr.height).toFixed(2)+'px';if(m.lineRatio)el.style.lineHeight=String(m.lineRatio);el.style.textShadow=scaleShadow(m.textShadow,factor);
    if(kind==='left'){el.style.left=(m.left*100).toFixed(4)+'%';el.style.right='auto';el.style.top=(m.top*100).toFixed(4)+'%'}
    else if(kind==='right'){el.style.right=(m.right*100).toFixed(4)+'%';el.style.left='auto';el.style.top=(m.top*100).toFixed(4)+'%'}
    else if(kind==='lyrics'&&m.width>0)el.style.width=(m.width*100).toFixed(4)+'%';
  }
  function enterFullscreen(){
    if(!last)capture(true);if(!last)return;
    if(!fullscreenRestore)fullscreenRestore={lyrics:saveInline(lyrics),brandL:saveInline(brandL),brandR:saveInline(brandR),title:saveInline(titleText)};
    requestAnimationFrame(()=>{const pr=preview.getBoundingClientRect();applyMetric(lyrics,last.lyrics,pr,'lyrics');applyMetric(brandL,last.brandL,pr,'left');applyMetric(brandR,last.brandR,pr,'right');applyMetric(titleText,last.title,pr,'title')});
  }
  ...
  function fullChange(){const active=document.fullscreenElement===preview||document.webkitFullscreenElement===preview;active?enterFullscreen():leaveFullscreen()}
  document.addEventListener('fullscreenchange',fullChange);document.addEventListener('webkitfullscreenchange',fullChange);
  document.getElementById('fullBtn')?.addEventListener('pointerdown',()=>capture(true),true);document.getElementById('fullBtn')?.addEventListener('click',()=>capture(true),true);

  const looksLikePreviewControl=el=>{const id=String(el?.id||'');return /(?:font|title|brand|corner|outline|size|plus|minus)/i.test(id)||!!el?.closest?.('#hksCornerBrandControls,#hksBrandOutlineControls97')};
  ...
  window.addEventListener('resize',()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>capture(),140)});document.getElementById('loadProject')?.addEventListener('change',()=>setTimeout(()=>capture(),900));

  function assStyle(ass,name,fn){const re=new RegExp('^(Style:\\s*'+name+',[^\\n]*)$','gmi');return ass.replace(re,line=>{const p=line.split(',');try{fn(p)}catch(_){}return p.join(',')})}
  const assSize=m=>m?Math.max(8,Math.min(600,Math.round((m.fontH||0)*TARGET_H))):0;
  ...
  function firstSync(duration){try{for(const w of words){const t=Number(w?.time);if(Number.isFinite(t))return Math.max(.05,Math.min(Number(duration)||t,t))}}catch(_){}return 0}
  ...
  try{
    ...
    if(typeof prev==='function'&&!prev.__hksPreviewParity152){
      const wrapped=function(duration){
        ...
        if(m.lyrics)ass=assStyle(ass,'Lyrics',p=>{if(p.length>22){p[2]=String(assSize(m.lyrics));p[18]='5'}});
        if(m.brandL)ass=assStyle(ass,'BrandL',p=>{if(p.length>22){p[2]=String(assSize(m.brandL));p[18]='7';p[19]=String(marginX(m.brandL.left));p[21]=String(marginY(m.brandL.top))}});
        if(m.brandR)ass=assStyle(ass,'BrandR',p=>{if(p.length>22){p[2]=String(assSize(m.brandR));p[18]='9';p[20]=String(marginX(m.brandR.right));p[21]=String(marginY(m.brandR.top))}});
        if(m.title)ass=assStyle(ass,'SongTitle',p=>{if(p.length>22){p[2]=String(assSize(m.title));p[18]='5'}});

        const title=currentTitle(),end=firstSync(duration),titleSize=assSize(m.title)||96;
        if(title&&end>0&&!/^Style:\s*SongTitle,/mi.test(ass)){
          const base=`Style: SongTitle,Noto Sans Hebrew,${titleSize},&H00FFFFFF,&H00FFFFFF,&H00000000,&H00000000,-1,0,0,0,100,100,0,0,1,5,0,5,70,70,30,1`;
          ass=ass.replace(/\n\[Events\]/,'\n'+base+'\n\n[Events]');
        }
        ...
        window.__hksLastAssMetrics152={lyrics:assSize(m.lyrics),brandL:assSize(m.brandL),brandR:assSize(m.brandR),title:assSize(m.title),brandLeft:m.brandL?marginX(m.brandL.left):0,brandRight:m.brandR?marginX(m.brandR.right):0,brandTopL:m.brandL?marginY(m.brandL.top):0,brandTopR:m.brandR?marginY(m.brandR.top):0};
        return ass;
      };
