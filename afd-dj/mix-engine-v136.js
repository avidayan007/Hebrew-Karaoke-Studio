(()=>{
 ...
 function install(){const d=D();if(!d||d.documentElement.dataset.afdMix136)return;const cross=d.getElementById('cross');if(!cross)return;d.documentElement.dataset.afdMix136='1';
  ...
  function start(k){const v=media(k),panel=v?.closest('.panel')||d.querySelector('.deck'+k),btn=panel?.querySelector('[data-act="play"]');if(d.getElementById('ytDeck'+k)){btn?.click();return}if(v?.src&&v.paused){v.play().catch(()=>btn?.click());return}if(btn&&!playing(k))btn.click()}
