(()=>{
 ...
 function apply(){
  ...
  /* Enough height for the mixer/volume faders; lower workspace starts after it, never over it. */
  const target=Math.max(475,Math.min(570,Math.round(vh*.55)));f.style.height=target+'px';f.style.overflow='hidden';
  try{const d=f.contentDocument;if(!d?.head)return;let s=d.getElementById('afdHalfLayoutV45');if(!s){s=d.createElement('style');s.id='afdHalfLayoutV45';d.head.appendChild(s)}s.textContent=`
   ...
   .center{gap:4px!important}.master{padding:4px!important}.mixer{gap:4px!important;grid-template-columns:1fr 52px 1fr!important;min-height:252px!important}.channel{padding:4px!important;min-height:246px!important}.vu{padding:4px!important}.vuCol{height:96px!important}
   .browser{display:none!important}
  `}catch(e){}
  const dock=document.querySelector('.dock'),view=document.querySelector('.view');if(dock){dock.style.marginTop='0';dock.style.position='relative';dock.style.zIndex='1'}if(view)view.style.height=Math.max(210,vh-target-145)+'px';
 }
