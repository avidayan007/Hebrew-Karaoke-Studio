(()=>{
 ...
 function loadLocal(deck,it){const win=W();if(!win||typeof win.load!=='function'||!it?.file)return;try{window.AFDYouTubeState?.clear?.(deck,it);window.AFDSpotifyState?.clear?.(deck)}catch(e){}win.load(deck,it.file);window.dispatchEvent(new CustomEvent('afd-local-load',{detail:{deck,item:it}}))}
 ...
 function frameClick(e){const b=e.target.closest?.('[data-a],[data-qdeck]');if(!b||b.dataset.afdOk158==='1')return;const k=b.dataset.a||b.dataset.qdeck;if(!['A','B'].includes(k)||!isPlaying(k))return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();ask(k,()=>{b.dataset.afdOk158='1';b.click();setTimeout(()=>delete b.dataset.afdOk158,0)})}
 ...
 function bindFrame(){const d=D();if(!d||d===boundDoc)return;boundDoc=d;d.addEventListener('click',frameClick,true);d.addEventListener('dragstart',frameDragStart,true);d.addEventListener('dragend',frameDragEnd,true);d.addEventListener('drop',frameDrop,true)}
