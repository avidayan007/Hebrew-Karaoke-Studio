...
const RUNTIME_KEY_JS=fs.readFileSync(path.join(__dirname,'runtime-v182.js'),'utf8');
const RUNTIME_MEDIA_PLAYLIST_JS=fs.readFileSync(path.join(__dirname,'runtime-v177.js'),'utf8');
const RUNTIME_LOAD_JS=fs.readFileSync(path.join(__dirname,'runtime-v183.js'),'utf8');
...
const MEDIA_PIPELINE_VERSION='v4';
let mediaCacheDir='';
...
function runFfmpeg(args,timeoutMs=600000){
 return new Promise((resolve,reject)=>{
  const cp=spawn(ffmpegExecutable(),args,{windowsHide:true,stdio:['ignore','ignore','pipe']});let err='',done=false;
  try{os.setPriority(cp.pid,os.constants.priority.PRIORITY_IDLE)}catch(e){try{os.setPriority(cp.pid,os.constants.priority.PRIORITY_BELOW_NORMAL)}catch(x){}}
  const finish=error=>{if(done)return;done=true;clearTimeout(timer);error?reject(error):resolve()};
  const timer=setTimeout(()=>{try{cp.kill('SIGKILL')}catch(e){};finish(new Error('FFmpeg preparation timed out.'))},timeoutMs);
  cp.stderr.on('data',b=>{err+=b.toString();if(err.length>16000)err=err.slice(-16000)});
  cp.on('error',e=>finish(e));cp.on('close',code=>finish(code===0?null:new Error(err.trim()||('FFmpeg exited '+code))))
 })
}
async function tryRemux(input,output){
 safeUnlink(output);
 try{await runFfmpeg(['-hide_banner','-loglevel','error','-nostdin','-y','-i',input,'-map','0:v:0?','-map','0:a:0?','-c','copy','-movflags','+faststart',output],90000);return fs.existsSync(output)&&fs.statSync(output).size>1024}catch(e){safeUnlink(output);return false}
}
async function transcodeVideo(input,output){
 safeUnlink(output);
 await runFfmpeg(['-hide_banner','-loglevel','error','-nostdin','-y','-threads','1','-filter_threads','1','-filter_complex_threads','1','-i',input,'-map','0:v:0?','-map','0:a:0?','-c:v','libx264','-preset','ultrafast','-tune','fastdecode','-crf','25','-threads','1','-pix_fmt','yuv420p','-c:a','aac','-b:a','160k','-max_muxing_queue_size','2048','-movflags','+faststart',output],900000)
}
async function transcodeAudio(input,output){
 safeUnlink(output);await runFfmpeg(['-hide_banner','-loglevel','error','-nostdin','-y','-threads','1','-i',input,'-vn','-c:a','pcm_s16le','-ar','48000','-ac','2',output],600000)
}
async function prepareMedia(meta){
 ...
 if(!force&&NATIVE_EXT.has(ext))return{url:mediaUrl(input),converted:false,remuxed:false,kind,name:path.basename(input)};
 const st=fs.statSync(input),sig=crypto.createHash('sha1').update(MEDIA_PIPELINE_VERSION+'|'+input+'|'+st.size+'|'+st.mtimeMs+'|'+kind).digest('hex');
 fs.mkdirSync(mediaCacheDir,{recursive:true});const output=path.join(mediaCacheDir,sig+(kind==='video'?'.mp4':'.wav')),jobKey=sig+'|'+kind;
 let remuxed=false;
 if(!fs.existsSync(output)||fs.statSync(output).size<1024){
  if(MEDIA_JOBS.has(jobKey))await MEDIA_JOBS.get(jobKey);
  else{
   const job=(async()=>{try{
    if(kind==='video'&&REMUX_FIRST.has(ext)){remuxed=await tryRemux(input,output);if(remuxed)return}
    if(kind==='video')await transcodeVideo(input,output);else await transcodeAudio(input,output)
   }catch(e){safeUnlink(output);throw e}})();
   MEDIA_JOBS.set(jobKey,job);try{await job}finally{MEDIA_JOBS.delete(jobKey)}
  }
 }
 if(!fs.existsSync(output)||fs.statSync(output).size<1024)throw new Error('Prepared media file was not created.');
 if(REMUX_FIRST.has(ext)&&!force&&!remuxed){try{remuxed=fs.statSync(output).mtimeMs>=st.mtimeMs&&false}catch(e){}}
 return{url:mediaUrl(output),converted:!remuxed,remuxed,kind,name:path.basename(input)}
}
async function savePlaylistFile(event,payload){if(!trustedSender(event))throw new Error('Playlist request blocked.');const tracks=playlistTracks(payload?.tracks);if(!tracks.length)throw new Error('Playlist has no available local tracks.');const base=cleanPlaylistName(payload?.defaultName||'AFD Playlist');const result=await dialog.showSaveDialog({title:'Save AFD DJ Playlist',defaultPath:path.join(app.getPath('documents'),base+'.afdplaylist'),buttonLabel:'Save Playlist',filters:[{name:'AFD DJ Playlist',extensions:['afdplaylist']},{name:'JSON',extensions:['json']}],properties:['showOverwriteConfirmation','createDirectory']});if(result.canceled||!result.filePath)return{canceled:true};let filePath=result.filePath;if(!/\.(afdplaylist|json)$/i.test(filePath))filePath+='.afdplaylist';const name=playlistTitle(filePath,base),data={format:'AFD-DJ-PLAYLIST',version:1,name,savedAt:new Date().toISOString(),tracks};fs.writeFileSync(filePath,JSON.stringify(data,null,2),'utf8');return{canceled:false,filePath,name,tracks}}
async function readPlaylistFile(event,filePath){if(!trustedSender(event))throw new Error('Playlist request blocked.');const p=path.resolve(String(filePath||''));if(!path.isAbsolute(p)||!fs.existsSync(p)||!fs.statSync(p).isFile())throw new Error('Playlist file not found.');const data=JSON.parse(fs.readFileSync(p,'utf8'));if(!Array.isArray(data?.tracks))throw new Error('Invalid AFD DJ playlist.');return{name:cleanPlaylistName(data.name||playlistTitle(p)),filePath:p,tracks:data.tracks.map(x=>({path:String(x?.path||''),name:String(x?.name||''),folder:String(x?.folder||''),kind:String(x?.kind||'music')}))}}
...
function create(){
 ...
 function changeZoom(delta){manualZoom=clamp(Math.round((manualZoom+delta)*100)/100,.65,1.55);applyZoom()}
 function resetZoom(){manualZoom=1;applyZoom()}
 async function injectWindowsRuntime(){try{await w.webContents.executeJavaScript('window.__afdSignalsmithSource='+JSON.stringify(SIGNALSMITH_JS),true);await w.webContents.executeJavaScript(RUNTIME_BASE_JS,true);await w.webContents.executeJavaScript(RUNTIME_FIX_JS,true);await w.webContents.executeJavaScript(RUNTIME_PERSIST_JS,true);await w.webContents.executeJavaScript(RUNTIME_LIBRARY_JS,true);await w.webContents.executeJavaScript(RUNTIME_LIBRARY_FIX_JS,true);await w.webContents.executeJavaScript(RUNTIME_TEXT_SCROLL_JS,true);await w.webContents.executeJavaScript(RUNTIME_KEY_JS,true);await w.webContents.executeJavaScript(RUNTIME_MEDIA_PLAYLIST_JS,true);await w.webContents.executeJavaScript(RUNTIME_LOAD_JS,true);return await w.webContents.executeJavaScript(RUNTIME_EJECT_JS,true)}catch(err){console.error('AFD runtime inject failed',err);return{ok:false,error:String(err)}}}
 ...
 w.loadURL('https://afd-dj.vercel.app/workstation.html?v=183&t='+Date.now());
...
app.whenReady().then(async()=>{
 mediaCacheDir=path.join(app.getPath('userData'),'media-cache-v4');try{fs.mkdirSync(mediaCacheDir,{recursive:true})}catch(e){}
 ...
 ipcMain.handle('afd-media-prepare',async(event,meta)=>{if(!trustedSender(event))throw new Error('Media request blocked.');return prepareMedia(meta)});ipcMain.handle('afd-playlist-save',savePlaylistFile);ipcMain.handle('afd-playlist-read',readPlaylistFile);
