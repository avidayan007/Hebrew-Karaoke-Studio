...
async function mixNow(i){if(busy)return;const arr=q(),it=arr[i];if(!it)return;const from=activeDeck(),to=from==='A'?'B':'A';busy=true;status('SIDEVIEW • מכין '+it.name+' ל-DECK '+to+'...');try{const ok=await loadTo(to,it);if(ok===false){status('SIDEVIEW • לא ניתן לטעון '+it.name);return}await waitSource(to,8000);await sleep(120);const mix=window.__afdWin188?.doMix;if(typeof mix!=='function'){status('SIDEVIEW • מנוע MIX עדיין נטען');return}status('SIDEVIEW • MIX NOW • '+it.name);await mix();}catch(e){status('SIDEVIEW MIX ERROR • '+(e?.message||e))}finally{busy=false}}
