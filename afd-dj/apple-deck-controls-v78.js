(()=>{
 ...
 async function transport(deck,action){try{const m=mk();if(!m||!state[deck]||state.active!==deck)return;if(action==='pause')await m.pause();if(action==='stop'){await m.pause();try{await m.seekToTime(0)}catch(e){}}}catch(e){console.warn('Apple deck transport',e)}}
 ...
 window.addEventListener('afd-deck-transport',e=>{const {deck,action}=e.detail||{};if((deck==='A'||deck==='B')&&(action==='pause'||action==='stop'))transport(deck,action)});
