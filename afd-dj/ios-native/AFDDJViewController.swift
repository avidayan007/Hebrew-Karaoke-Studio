...
final class AFDDJViewController: UIViewController, WKScriptMessageHandler, WKNavigationDelegate {
    ...
    private func installNativeBridgeJavaScript(){
        let js=#"""
        (()=>{window.AFDNative=true;window.AFDChooseFolder=()=>window.webkit?.messageHandlers?.afdNative?.postMessage({action:'chooseFolder'});const hook=()=>{const b=document.getElementById('folderBtn');if(!b||b.dataset.afdNativeHook==='1')return;b.dataset.afdNativeHook='1';b.textContent='📁 הוסף תיקייה';b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();window.AFDChooseFolder()},true)};hook();new MutationObserver(hook).observe(document.documentElement,{childList:true,subtree:true})})();
        """#
