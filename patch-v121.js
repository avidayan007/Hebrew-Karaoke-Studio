...
(function(){
  ...
  // Clicking an empty place on the synchronization waveform ONLY selects a return point.
  // Existing orange marker drags remain pure timing edits and never arm a re-sync.
  canvas.addEventListener('pointerdown',e=>{
    markerGesture=nearMarker(e.clientX);
  },true);
  ...
  canvas.addEventListener('pointerup',()=>{
    ...
    if(wasMarker)return;
    setTimeout(()=>{
      armedTime=Math.max(0,Number(audio.currentTime)||0);
  ...
  function commitResync(){
    ...
    try{
      ...
      // Keep every synchronized word through the last word at/before the cursor.
      let last=-1;
      for(let i=0;i<words.length;i++){
        if(timed(words[i])&&Number(words[i].time)<=t)last=i;
      }
      const next=Math.min(last+1,words.length);

      // Delete ONLY the synchronization after the cursor. Everything before it stays untouched.
      ...
      setTimeout(()=>{
        try{current=next;renderWords();updateSyncPreview();window.__hksDrawSyncWave?.()}catch(_){}
      },80);
      ...
      console.error('[v121 commit resync]',e);
      return false;
    }
  }

  // This exact button is the commit point. Normal Play does NOT erase synchronization.
  syncPlay.addEventListener('click',()=>{commitResync()},true);

  // Each subsequent ◆ Sync press uses the normal sync() flow: current word gets the new time, then advances to the next word.
