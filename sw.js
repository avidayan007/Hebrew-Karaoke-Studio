...
const ASSETS=['./','index.html','app.js?v=30','patch-v17.js?v=30','patch-v18.js?v=30','patch-v20.js?v=30','patch-v21.js?v=30','patch-v25.js?v=30','patch-v27.js?v=30','patch-v28.js?v=30','patch-v29.js?v=30','patch-v30.js?v=141',...PATCHES.map(n=>`patch-v${n}.js?v=141`),'manifest.webmanifest','433A5E98-4A3F-40B9-A6D0-91B22FF5B848.png'];
self.addEventListener('install',e=>{self.skipWaiting();e.waitUntil((async()=>{const c=await caches.open(CACHE);await Promise.allSettled(ASSETS.map(async a=>{try{const r=await fetch(a,{cache:'reload'});if(r&&r.ok)await c.put(a,r.clone())}catch(_){}}))})())});
...
self.addEventListener('fetch',e=>{
  ...
  if(isPatch){
    e.respondWith((async()=>{
      ...
      if(hit)return hit;
      try{const r=await fetch(e.request,{cache:'reload'});if(r&&r.ok)await c.put(e.request,r.clone());return r}catch(err){const stale=await caches.match(e.request,{ignoreSearch:true});if(stale)return stale;throw err}
    })());return;
  }
