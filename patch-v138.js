...
(function(){
  ...
  async function hardUpdate(btn){
    ...
    try{
      ...
      if('serviceWorker' in navigator){
        try{const reg=await navigator.serviceWorker.register('sw.js?v=140',{updateViaCache:'none'});await reg.update()}catch(_){}
        for(let i=0;i<12;i++){if(await pingSW()===SW)break;await sleep(350)}
      }
      const u=new URL(location.href);u.searchParams.set('hksUpdate','140');u.searchParams.set('_',Date.now().toString());location.replace(u.href);
    }catch(e){
  ...
  function bindUpdateButton(){
    ...
    btn.dataset.hksUpdate138='1';btn.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();hardUpdate(btn)},true);
  }
  ...
  const ver=$('.version');if(ver&&Number(window.__hksLoaderVersion||0)<140)ver.textContent='Web v1.138';
